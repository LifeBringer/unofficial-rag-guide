...
def retrieve_bm25(query: str, k: int = 5) -> list[dict]:
    ...
    return [{**chunks[i], "text": chunks[i]["text"], "bm25": round(scores[i], 2),
             "distance": None} for i in ranked[:k]]


def retrieve_hybrid(query: str, k: int = 5) -> list[dict]:
    """Top-k chunks by RRF fusion of semantic (cosine) and BM25 rankings."""
    semantic = retrieve(query, k=POOL)
    keyword = retrieve_bm25(query, k=POOL)
